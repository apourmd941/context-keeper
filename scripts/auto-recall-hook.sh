...
# C4: opt-in local-caller auth. Read the daemon's local token (0600 file under
# the data dir) and send it as a Bearer header. Harmless when the daemon isn't
# enforcing (header ignored); required once `require_token` is on. Stay
# fail-silent: a missing/unreadable token simply means no header.
CK_DATA_DIR="${CK_DATA_DIR:-$HOME/.context-keeper}"
TOKEN_FILE="$CK_DATA_DIR/state/local-token"
AUTH_TOKEN=""
if [ -r "$TOKEN_FILE" ]; then
  AUTH_TOKEN=$(tr -d '\n\r' < "$TOKEN_FILE" 2>/dev/null) || AUTH_TOKEN=""
fi
...
# C2: send `cwd` as a candidate path so glob-scoped memories ("inject when a
# file matching **/*.rs is in play") can fire. The daemon also harvests
# path-like tokens from the prompt text itself, so a bare cwd is enough here —
# we keep the hook body minimal and let the server do the parsing.
req_body=$(jq -nc \
  ...
  '{query: $q, source: "hook"}
   ...
   + (if $cwd == "" then {} else {paths: [$cwd]} end)')
