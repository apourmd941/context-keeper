...
export default function App() {
  ...
  const [memoriesOpen, setMemoriesOpen] = useState(
    params?.get('memories') === '1',
  );
  ...
  const searchContent = (q: string) => {
    ...
    // The three right-side panels are mutually exclusive (same stacking).
    setSettingsOpen(false);
    setMemoriesOpen(false);
    setRecallOpen(true);
  };
      ...
      <header className="px-5 py-2.5 flex items-center gap-3 shrink-0 border-b border-ink-700/60 bg-ink-900/70 backdrop-blur-sm">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            ...
            title="Memories — view, pin, and author your standing rules"
            aria-label="memories"
