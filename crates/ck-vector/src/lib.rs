...
impl VectorStore {
    ...
    /// Drop every record belonging to `session_id` whose chunk id is NOT in
    /// `keep`. Used on re-index so stale chunks (e.g. ones removed when
    /// redaction shifted chunk boundaries) leave no dead embedding behind.
    /// Returns the number removed. Rebuilds the id→index map after compaction.
    pub fn prune_session(
        ...
        self.records
            .retain(|r| r.session_id != session_id || keep.contains(&r.chunk_id));
