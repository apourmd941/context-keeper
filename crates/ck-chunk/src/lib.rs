...
fn make_chunk(
    ...
    // Scrub credentials BEFORE counting tokens, embedding, or storing — this
    // is the single chokepoint every chunk passes through, so no secret ever
    // reaches a derived artifact (embedding, store, recall result, or UI).
    let (clean, n) = redact::redact_secrets(&text);
    if n > 0 {
