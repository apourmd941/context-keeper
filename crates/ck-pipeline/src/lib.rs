...
pub fn index_file(
    ...
    // Prune stale chunks from a previous index of this session. Re-chunking
    // can produce a different chunk set (notably when secret redaction shifts
    // chunk boundaries), and an un-pruned orphan would keep the old, possibly
    // secret-bearing text on disk + a dead embedding in the index. Remove from
    // all three stores: chunk JSON files, the vector store, and the meta index.
    let keep: HashSet<String> = chunks.iter().map(|c| c.id.0.clone()).collect();
    match ck_store::prune_session_chunk_files(&state.layout, &session.id, &keep) {
        Ok(removed) if !removed.is_empty() => {
            if let Ok(mut store) = state.vector.write() {
                let _ = store.prune_session(&session.id.0, &keep);
            }
            if let Ok(mut meta) = state.meta.lock() {
                let _ = meta.prune_session_chunks(&session.id.0, &keep);
            }
            tracing::debug!(session = %session.id.0, pruned = removed.len(), "pruned stale chunks");
        }
        Ok(_) => {}
        Err(e) => warn!(session = %session.id.0, error = %e, "chunk prune failed"),
    }
